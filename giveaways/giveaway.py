...
# -------------------------------------Functions---------------------------------
    ...
    async def start_giveaway(self, messageid: int, info):
        ...
        while True:
            ...
            e.set_footer(text=f"Winners: {0} | Ends at".format(info["winners"])
    ...
    async def end_giveaway(self, messageid: int, info, reroll: int=-1):
        ...
        message=self.message_cache.get(str(messageid), self.bot._connection._get_message(int(messageid))
        if not message:
            try:
                message=await channel.fetch_message(messageid)
            except discord.NotFound:
        ...
        if len(final_list) == 0:
            host=info["host"] if message.guild.get_member(
                info["host"]) is not None else "Host Not Found"
                e=discord.Embed(
                    title=title, description=f"Host: <@{host}> \n Winners: None")
                if info["requirement"]:
                    requirements=[]
                    for r in info["requirement"]:
...
# -------------------------------------gset---------------------------------
    ...
    async def add(self, ctx, role: Optional[discord.Role]=None):
    ...
    async def remove(self, ctx, role: Optional[discord.Role] = None):
        roles = await self.config.guild(ctx.guild).bypassrole()
    ...
    async def cmd_add(self, ctx, role: Optional[discord.Role]=None):
        """Add a role to blacklist""
        roles = await self.config.guild(ctx.guild).blacklisted()
        if role.id in roles:
         return await ctx.send("This role is already blacklisted")
        roles.append(role.id)
        await self.config.guild(ctx.guild).blacklisted.set(roles)
        await ctx.send("Added to the blacklisted roles")
    
    @bypassrole.command(name="remove")
    async def cmd_remove(self, ctx, role: Optional[discord.Role] = None):
    ...
    async def settings(self, ctx):
        ...
        e.add_field(name="Manager Role",
                    value="<@{0}>".format(data["manager"]))
        e.add_field(name="Bypass Roles", value=bypass_roles)
        e.add_field(name="Blacklisted Roles", value=blacklisted_roles)
        e.add_field(name="Default Requirement",
                    value="<@{0}>".format(data["default_req"]))
        e.add_field(name="Total Giveaways",
                    value=len(data["giveaways"].keys()))
        ...
        e.add_field(name="Autodelete invocation messages",
                    value=data["delete"])
        e.add_field(name="Pingrole", value="<@{0}".format(data["pingrole"]))
        ...
        e.add_field(name="Start Header", value="```\n{0}```".format(data["startHeader"]), inline=False)
        e.add_field(name="End Header", value="```\n{0}```".format(data["endHeader"]), inline=False)
        e.add_field(name="Description", value="```\n{0}```".format(data["description"]), inline=False)
...
# -------------------------------------gstore---------------------------------
    ...
    data = await self.config.guild(message.guild).all()
        ...
        bypassrole = await self.config.guild(message.guild).bypassrole()
        if not (await self.can_join(user, gaws[str(messageid)]):
        e=discord.Embed(title="Missing Giveaway Requirement",
                                    description=f"You do not have the `{r.name}` role which is required for [this]({message.jump_url}) giveaway or you have a blacklisted role. You can check gset settings to see if you have the blacklisted role")
