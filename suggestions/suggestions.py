...
class Suggestions(commands.Cog):
    ...
    async def decision(self, ctx, channel: Optional[discord.TextChannel] = None):
        ...
        if not channel:
            ...
            await ctx.send(
                "I will now post suggestion decisions in the submission channel"
            )
        else:
    ...
    async def suggestions_directmessage(self, ctx, dm: Optional[bool] = None):
        ...
        if dm is None:
            await ctx.send(
                "You need to specify either True or False after this command."
            )
        else:
            await self.config.guild(ctx.guild).dm.set(dm)
            if dm:
                await ctx.send(
                    "I will now directmessage users on suggestion decisions."
                )
            else:
                await ctx.send(
                    "I will no longer directmessage users on suggestion decisions."
                )
    ...
    async def suggestions_edit(self, ctx, edit: Optional[bool] = None):
        ...
        if edit is None:
            ...
            if edit:
                await ctx.send(
                    "I will now edit messages instead of reposting them (Still send in decision channel if there is one)."
                )
            else:
                await ctx.send(
                    "I will no longer edit messages and instead repost them."
                )
    ...
    async def suggestions_blacklist(self, ctx, member: Optional[discord.Member] = None):
        ...
        if not member:
            ...
            await ctx.send(
                f"**{member.name}** has been blacklisted from making suggestions in this server."
            )
    ...
    async def anonymous(self, ctx, anon: Optional[bool] = None):
        ...
        if anon is None:
            ...
            if anon:
                await ctx.send(
                    "Suggestion authors will no longer show up on suggestions."
                )
            else:
    ...
    async def who(self, ctx, number: Optional[int] = None):
        ...
        if number not in suggestions:
            ...
            await ctx.send(
                f"**{author.name}** made suggestion {number}, the content was `{content}`"
            )
    ...
    async def suggest(self, ctx, *, content=None):
        ...
        if not channel.permissions_for(ctx.me).send_messages:
            await ctx.send(
                "Uh oh, I do not have permissions to send messages in the suggestion channel."
            )
            return
        ...
        await ctx.author.send(
            f"Your suggestion has been sent for approval. You can view it here: {message.jump_url}"
        )
    ...
    @commands.mod_or_permissions(manage_guild=True)
    async def approve(
        self, ctx, number: Optional[int] = None, *, reason="No Reason Provided"
    ):
        ...
        try:
            ...
            await ctx.send(
                "Uh oh, I couldn't find this message. Check if you changed your suggestion channel."
            )
            return
        e = discord.Embed(
            title=f"Suggestion #{number} approved.",
            description=content,
            color=discord.Color.green(),
        )
    ...
    async def reject(
        ...
        try:
            ...
            return
        e = discord.Embed(
            title=f"Suggestion #{number} denied.",
            description=content,
