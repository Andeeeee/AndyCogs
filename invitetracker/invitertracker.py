...
class InviteTracker(commands.Cog):
    ...
    def __init__(self, bot: Red):
        ...
        self.config = Config.get_conf(
            self, 160805014090190130501014, True 
        )
    ...
    async def get_inviter(self, member: discord.Member) -> str:
        ...
        if member.guild.me.guild_permissions.manage_guild and "VANITY_URL" in member.guild.features:
            try:
        ...
        if (member.guild.me.guild_permissions.manage_guild and member.guild.me.guild_permissions.view_audit_log) and not link:
    ...
    async def who(self, ctx, user: Optional[discord.Member] = None):
        ...
        if not inviter:
            return await ctx.send(f"**{user}** either joined from a vanity invite or I couldn't track this")
        elif ctx.guild.get_member(inviter) is None:
            return await ctx.send(f"**{user}**'s inviter seems to have left the server, the id is {inviter}")
        else:
            inviter = ctx.guild.get_member(inviter)
            await ctx.send(f"**<@{inviter.display_name}>** invited **{user}**")
    
    @invites.command()
    async def top(self, ctx, amount: Optional[int] = 10, top_to_bottom: Optional[bool] = True):
        """View the top/bottom inviters"""
        all_members = await self.config.all_members(ctx.guild)
        member_data = [(member, data["invites"]) for member, data in all_members if data["count"] > 0 and ctx.guild.get_member(int(member)) is not None]
        sorted_data = sorted(member_data, key = lambda k: k[1], reverse=top_to_bottom)[:amount]
    ...
    async def on_member_join(self, member: discord.Member):
        ...
        if not inviter:
            if not channel:
                ...
                await channel.send(f"I couldn't figure out how **{member.display_name}** joined")
        else:
            await self.config.member(member).inviter.set(inviter.id)
            async with self.config.member(inviter).invites() as invites:
                invites += 1
    ...
    async def on_member_leave(self, member: discord.Member):
        ...
        if not inviter:
            if not channel:
                ...
                await channel.send(f"I couldn't figure out who inivited **{member.name}**")
        else:
            async with self.config.member_from_ids(member.guild.id, inviter).invites() as invites:
                invites -= 1
                inviter_invites = invites
            if not channel:
                return 
