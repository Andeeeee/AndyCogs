...
class DankUtilities(commands.Cog):
    ...
    async def tradeset_channel(
        ...
        if not channel:
            ...
            await ctx.send(
                "I've cleared the channel. Trade ads will now post in the channel the command was used in."
            )
        else:
    ...
    async def trade(
        self, ctx, user: Optional[discord.Member] = None, *, offer: str = None
    ):
        """Trade with another user"""
        if not user:
            await ctx.send(
                "You can't trade with nobody. Specify a members ID, name, or mention after this please."
            )
        ...
        await ctx.send(
            f"{user.mention}: {ctx.author.mention} wants to trade {offer}. Do you accept?"
        )
